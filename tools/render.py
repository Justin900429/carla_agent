...
class MapImage:
    def __init__(self, carla_world, carla_map, pixels_per_meter):
        ...
        # Render map
        self.draw_road_map(
            self.big_map_surface, carla_world, carla_map, self.world_to_pixel, self.world_to_pixel_width
        )
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def draw_broken_line(surface, color, closed, points, width):
            broken_lines = [x for n, x in enumerate(zip(*(iter(points),) * 20, strict=False)) if n % 3 == 0]
        ...
        def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign):
            margin = 0.25
            marking_1 = [
                world_to_pixel(lateral_shift(w.transform, sign * w.lane_width * 0.5)) for w in waypoints
            ]
            ...
            for waypoint in topology:
                ...
                for w in waypoints:
                    ...
                    while right_lane and right_lane.lane_type != carla.LaneType.Driving:
                        if right_lane.lane_type == carla.LaneType.Shoulder:
                            shoulder[1].append(right_lane)

                        if right_lane.lane_type == carla.LaneType.Parking:
                            parking[1].append(right_lane)

                        if right_lane.lane_type == carla.LaneType.Sidewalk:
                            sidewalk[1].append(right_lane)
...
class BirdeyeRender:
    def __init__(
        ...
        pixels_per_meter = screen_size / obs_range
        self.pixels_ahead_vehicle = (obs_range / 2 - d_behind) * pixels_per_meter
        self.map_image = MapImage(
            carla_world=self.world_manager.world,
            carla_map=self.town_map,
            pixels_per_meter=pixels_per_meter,
        )
        self.original_surface_size = min(screen_size, screen_size)
    ...
    def _split_actors(self):
        ...
        if self.hero_actor is not None and len(vehicles) > 1:
            ...
            for n, _ in enumerate(sorted(vehicle_list, key=distance)):
                if n > 15:
                    break
    ...
    def _render_hist_actors(self, surface, actor_polygons, actor_type, world_to_pixel, num):
        ...
        all_ego_ids = set(self.world_manager.get_all_ego_ids())

        for ID, poly in actor_polygons.items():
            corners = []
            for p in poly:
                corners.append(carla.Location(x=p[0], y=p[1]))
            corners.append(carla.Location(x=poly[0][0], y=poly[0][1]))
            corners = [world_to_pixel(p) for p in corners]
            color_value = max(0.8 - 0.8 / lp, 0)
    ...
    def _get_actor_polygons(self, filt):
        actor_poly_dict = {}
        for actor in self.world_manager.world.get_actors().filter(filt):
    ...
    def render(self, render_types=None):
        ...
        self.render_actors(
            self.actors_surface,
            self._get_actor_polygons("vehicle.*"),
    ...
    def display_to_rgb(self, display, obs_size):
        rgb = np.fliplr(np.rot90(display, 3))
        rgb = skimage.transform.resize(rgb, (obs_size, obs_size))
